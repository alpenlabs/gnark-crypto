...
// Modulus q =
//
//	q[base10] = 3450873173395281893717377931138512760570940988862252126328087024741343
//	q[base16] = 0x8000000000000000000000000000069d5bb915bcd46efb1ad5f173abdf
//
// # Warning
//
...
const (
	...
	Bits  = 232 // number of bits needed to represent a Element
	Bytes = 32  // number of bytes needed to represent a Element
)

// Field modulus q
const (
	q0 uint64 = 7997015069507693535
	q1 uint64 = 1861867133058260
	q2 uint64 = 0
	q3 uint64 = 549755813888
)
...
const qInvNeg uint64 = 11714297528646643681

func init() {
	_modulus.SetString("8000000000000000000000000000069d5bb915bcd46efb1ad5f173abdf", 16)
}
...
func (z *Element) SetInterface(i1 interface{}) (*Element, error) {
	if i1 == nil {
		return nil, errors.New("can't set sect.Element with <nil>")
	}
	...
	default:
		return nil, errors.New("can't set sect.Element from type " + reflect.TypeOf(i1).String())
	}
}
...
func (z *Element) SetOne() *Element {
	z[0] = 14058042853959117791
	z[1] = 5229930283032626846
	z[2] = 18446744073709548229
	z[3] = 549755813887
	return z
}
...
func (z *Element) IsOne() bool {
	return ((z[3] ^ 549755813887) | (z[2] ^ 18446744073709548229) | (z[1] ^ 5229930283032626846) | (z[0] ^ 14058042853959117791)) == 0
}
...
func (z *Element) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 3998507534753846768, 0)
	_, b = bits.Sub64(_z[1], 930933566529130, b)
	_, b = bits.Sub64(_z[2], 0, b)
	_, b = bits.Sub64(_z[3], 274877906944, b)
...
var rSquare = Element{
	1662017447374785462,
	17862379558768630662,
	10648783234889255434,
	385452267522,
}
...
func (z *Element) SetBytesCanonical(e []byte) error {
	if len(e) != Bytes {
		return errors.New("invalid sect.Element encoding")
	}
	v, err := BigEndian.Element((*[Bytes]byte)(e))
	if err != nil {
		return err
...
func (bigEndian) Element(b *[Bytes]byte) (Element, error) {
	...
	if !z.smallerThanModulus() {
		return Element{}, errors.New("invalid sect.Element encoding")
	}
...
func init() {
	_bLegendreExponentElement, _ = new(big.Int).SetString("4000000000000000000000000000034eaddc8ade6a377d8d6af8b9d5ef", 16)
	const sqrtExponentElement = "200000000000000000000000000001a756ee456f351bbec6b57c5ceaf8"
...
const (
	inversionCorrectionFactorWord0 = 4500346669753981940
	inversionCorrectionFactorWord1 = 16442028975296248623
	inversionCorrectionFactorWord2 = 16962653478030955613
	inversionCorrectionFactorWord3 = 288988251180
	invIterationsN                 = 16
)
